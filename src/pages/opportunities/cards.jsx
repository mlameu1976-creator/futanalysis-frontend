// src/pages/opportunities/cards.jsx
...
const CARDS_MARKETS = [
  ...
  { key: "OVER 4.5", label: "Over 4.5"  },
  { key: "UNDER 3.5",label: "Under 3.5" },
  { key: "UNDER 4.5",label: "Under 4.5" },
];
...
function CardOppCard({ item, index = 0 }) {
  const color = MARKET_COLOR[item.market] || T.yellow;
  ...
  return (
    <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: "10px", overflow: "hidden", transition: "border-color 0.2s, transform 0.2s" }}
      ...
      <div style={{ padding: "1rem" }}>
        ...
        {/* Média de cartões por time — evita que um time indisciplinado "mascare" o outro */}
        {(item.home_stats || item.away_stats) ? (
          <div style={{ display: "flex", justifyContent: "space-between", background: T.surfaceHi, borderRadius: "6px", padding: "0.4rem 0.6rem", marginBottom: "0.5rem" }}>
            ...
            <span style={{ fontSize: "0.62rem", color: T.textDim }}>cartões/jogo</span>
            <span style={{ fontSize: "0.68rem", color: T.textMid }}>
        ...
        {item.detail && <div style={{ fontSize: "0.7rem", color: T.yellow, marginBottom: "0.6rem" }}>📊 {item.detail}</div>}
...
export default function CardsOpportunitiesPage() {
  ...
  async function fetchHistory(group, period) {
    ...
    try {
      ...
      const res = await fetch(`${API}/history?group=cartoes&period=${p}`);
  ...
  async function fetchData() {
    ...
    try {
      const params = new URLSearchParams({ date: "all", min_probability: minProb, scope: "cards", limit: 100 });
  ...
  // filtro 100% local — sem refetch ao trocar mercado
  ...
  const btnP = { background: T.yellow, color: "#000", border: "none", borderRadius: "6px", padding: "0.45rem 1.1rem", fontWeight: 700, fontSize: "0.78rem", fontFamily: T.font, cursor: "pointer" };

  return (
    <>
      <Head>
        <title>Oportunidades de Cartões — FutAnalysis | Análise Estatística</title>
        <meta name="description" content="Oportunidades de mercado de cartões no futebol calculadas por modelo estatístico. Over/Under 3.5 e 4.5 cartões por jogo para as principais ligas do mundo." />
        <link rel="canonical" href="https://futanalysis.com.br/opportunities/cards" />
      </Head>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "2rem", paddingBottom: "1rem", borderBottom: `1px solid ${T.border}` }}>
        ...
        <h1 style={{ margin: 0, fontSize: "1.3rem", fontWeight: 700, color: T.yellow }}>🟨 Cartões</h1>
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.2rem", borderBottom: `1px solid ${T.border}`, paddingBottom: "0.8rem" }}>
        {[["opps","🃏 Cartões"],["history","📊 Histórico de Acertos"]].map(([tab, label]) => (
          <button key={tab} onClick={() => { setActiveTab(tab); if(tab==="history") fetchHistory("cartoes"); }} style={{
      ...
      {activeTab === "opps" && <>
      {/* Introdução */}
        <div style={{ background: T.surfaceHi, borderRadius: "10px", padding: "1rem 1.2rem", marginBottom: "1.5rem", borderLeft: `3px solid ${T.yellow}` }}>
          <p style={{ margin: 0, fontSize: "0.78rem", color: T.textMid, lineHeight: 1.7 }}>
            Oportunidades de cartões calculadas com base no <strong style={{ color: T.text }}>histórico dos últimos 15 jogos</strong> de cada time, usando distribuição de Poisson com peso exponencial. Mercados: Over/Under 3.5 e Over/Under 4.5 cartões por partida.
          </p>
        </div>
      ...
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "1.2rem" }}>
        {CARDS_MARKETS.map(m => (
          <button key={m.key} onClick={() => setMarket(m.key)} style={{
            background: market === m.key ? T.yellow : T.surface,
            color:      market === m.key ? "#000"   : T.textMid,
            border: `1px solid ${market === m.key ? T.yellow : T.border}`,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "1rem" }}>
        {filtered.map((item, i) => <CardOppCard key={`${item.match_id}-${item.market}-${i}`} item={item} index={(1 + i) % 10} />)}
      </div>
    </>
    }
    </div>

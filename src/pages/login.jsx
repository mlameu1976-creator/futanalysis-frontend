...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    setError("");
    try {
      const res = await fetch("https://futanalysis.com.br/api/auth/login", {
        method: "POST",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f172a, #1e293b)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter, sans-serif" }}>
      <div style={{ background: "#1e293b", padding: "40px", borderRadius: "16px", width: "100%", maxWidth: "400px", border: "1px solid #334155" }}>
        ...
        <p style={{ color: "white", textAlign: "center", opacity: 0.7, marginBottom: "32px" }}>Entrar na sua conta</p>
        ...
        <form onSubmit={handleLogin}>
          ...
          <button type="submit" disabled={loading}
            style={{ width: "100%", padding: "12px", background: "linear-gradient(135deg, #0ea5e9, #2563eb)", color: "white", border: "none", borderRadius: "8px", fontSize: "16px", fontWeight: "700", cursor: "pointer" }}>
            {loading ? "Entrando..." : "Entrar"}
          </button>
        </form>

        <p style={{ color: "white", textAlign: "center", marginTop: "24px", opacity: 0.7, fontSize: "14px" }}>
          Nao tem conta? <Link href="/register" style={{ color: "#38bdf8" }}>Cadastre-se</Link>
        </p>
